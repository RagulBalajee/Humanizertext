  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      body { background: radial-gradient(1000px 600px at 10% 10%, rgba(124,58,237,.18), transparent), radial-gradient(900px 500px at 90% 0%, rgba(6,182,212,.18), transparent), linear-gradient(180deg, #0b1020, #0d1226); color: var(--text); font-family: Inter, ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, "Apple Color Emoji", "Segoe UI Emoji"; margin: 0; }
      .glow {
        position: fixed; inset: -20vh; pointer-events: none; background: radial-gradient(50% 50% at 50% 50%, rgba(124,58,237,.12), transparent 60%);
        filter: blur(60px); opacity: .7; animation: float 12s ease-in-out infinite;
      }
      ...
      .panel { backdrop-filter: saturate(140%) blur(8px); background: var(--panel); border: 1px solid var(--border); border-radius: 16px; padding: 16px; box-shadow: var(--shadow); }
      .split { display: grid; grid-template-columns: 1fr; gap: 16px; }
      @media (min-width: 960px) { .split { grid-template-columns: 1fr 1fr; } }
      textarea { width: 100%; min-height: 360px; resize: vertical; background: #0b1222; color: var(--text); border: 1px solid var(--border); border-radius: 12px; padding: 14px; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, "Liberation Mono", "Courier New", monospace; font-size: 14px; box-shadow: inset 0 0 0 1px rgba(255,255,255,.02); }
      textarea:focus { outline: none; box-shadow: 0 0 0 2px rgba(124,58,237,.5); }
      .controls { display: flex; gap: 12px; flex-wrap: wrap; align-items: center; justify-content: space-between; }
      .btn { background: linear-gradient(90deg, var(--accent), var(--accent-2), var(--accent-3)); color: white; padding: 12px 18px; border: none; border-radius: 12px; cursor: pointer; font-weight: 700; letter-spacing: .2px; transition: transform .06s ease, filter .2s ease; box-shadow: 0 8px 20px rgba(124,58,237,.25); }
