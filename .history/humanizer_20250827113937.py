...
class TextHumanizer:
    ...
    def humanize_text(self, text):
        ...
        for attempt in range(10):
            ...
            # Step 2: Apply word-level transformations
            ...
            for sentence in sentences:
                ...
                for word, pos_tag in pos_tags:
                    # Higher probability of synonym replacement for longer words
                    if word.isalpha() and len(word) > 4 and random.random() < 0.4:
            ...
            # If similarity is too high, apply more aggressive transformations
            if similarity > self.target_similarity_range[1]:
                # Add more sentence restructuring
                if len(restructured_sentences) > 1:
                    random.shuffle(restructured_sentences)
                
                # Add more informal language
                humanized_text = self.add_human_patterns(humanized_text)
            ...
            # If similarity is too low, apply less aggressive transformations
            elif similarity < self.target_similarity_range[0]:
                # Keep more original words
                words = word_tokenize(original_text)
                pos_tags = nltk.pos_tag(words)
                
                new_words = []
                for word, pos_tag in pos_tags:
                    if word.isalpha() and len(word) > 6 and random.random() < 0.2:
                        new_word = self.synonym_replace(word, pos_tag)
                        new_words.append(new_word)
                    else:
                        new_words.append(word)
                
                humanized_text = " ".join(new_words)
        
        # If we can't achieve target similarity after 10 attempts, return the best result
