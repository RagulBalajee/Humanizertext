...
class TextHumanizer:
    ...
    def _split_list_blocks(self, text: str):
        lines = text.splitlines()
        blocks = []
        current = []
        ...
        for line in lines:
            ...
            if current_is_list is None:
                current_is_list = is_list
                current.append(line)
            elif is_list == current_is_list:
                current.append(line)
            else:
                blocks.append((current_is_list, "\n".join(current).strip()))
                current = [line]
                current_is_list = is_list
        if current:
            blocks.append((current_is_list, "\n".join(current).strip()))
    ...
    def _humanize_list_block(self, block: str) -> str:
        ...
        for line in block.splitlines():
            ...
            if m:
                indent, marker, item = m.group(1), m.group(2), m.group(3)
                humanized_item, _ = self.humanize_text(item)
                humanized_item = self._capitalize_first_letter(humanized_item)
                out_lines.append(f"{indent}{marker} {humanized_item}")
            else:
                # Fallback: treat as paragraph line within list block
                humanized_line, _ = self.humanize_text(line)
                out_lines.append(self._capitalize_first_letter(humanized_line))
        return "\n".join(out_lines)

    def _looks_like_implicit_list(self, content: str) -> bool:
        # Consider as implicit list if there are 3+ short paragraphs separated by blank lines
        paras = [p.strip() for p in content.split("\n\n") if p.strip()]
        if len(paras) < 3:
            return False
        short_count = sum(1 for p in paras if len(p.split()) <= 25)
        return short_count >= max(3, len(paras) // 2)

    def _humanize_implicit_list(self, content: str) -> str:
        paras = [p.strip() for p in content.split("\n\n") if p.strip()]
        out = []
        for p in paras:
            humanized, _ = self.humanize_text(p)
            humanized = self._capitalize_first_letter(humanized)
            out.append(f"- {humanized}")
        return "\n".join(out)
    ...
    def humanize_with_headings(self, text: str):
        ...
        for heading, content in sections:
            if content:
                # Preserve explicit lists; otherwise detect implicit list blocks
                if any(self._is_list_item(l) for l in content.splitlines() if l.strip()):
                    blocks = self._split_list_blocks(content)
                    humanized_blocks = []
                    for is_list, block in blocks:
                        ...
                        if is_list:
                            ...
                            if self._looks_like_implicit_list(block):
                                humanized_blocks.append(self._humanize_implicit_list(block))
                            else:
                                humanized_block, _ = self.humanize_text(block)
                                humanized_blocks.append(humanized_block)
                    humanized_content = "\n".join(humanized_blocks).strip()
                else:
                    if self._looks_like_implicit_list(content):
                        humanized_content = self._humanize_implicit_list(content)
                    else:
